...
shift=nu_max;
A=op.Trunc*(s*op.H+(1-s)*op.H0-shift*op.Id)*op.Mat_BC;
B=op.Trunc*op.Mat_BC;
small_nb_eigs = min(small_nb_eigs,size(A,1));
...
if nb_neg<2
    if small_nb_eigs < size(A,1)
        delta_s=delta_s_predictor(s,nu_max,tol_nu,op,small_nb_eigs+2,pert,delta_s_min,correc,show);
        return
    else
        error("We are already computing all the eigenvalues. Consider increasing K.")
    end
end
...
s_pert=s+pert;
A=op.Trunc*(s_pert*op.H+(1-s_pert)*op.H0-shift*op.Id)*op.Mat_BC;
B=op.Trunc*op.Mat_BC;
